/* vim sts=4 ts=4 noexpandtab sw=4 */
/**********************************************************************
...
const YYDEBUG = true
const YYERROR_VERBOSE = true
const YYSTACK_USE_ALLOCA = false

// TODO: follow all includes
// #include "ruby/ruby.h"
// #include "ruby/st.h"
...
// TODO: follow rb_parser_malloc
#define YYMALLOC(size)		rb_parser_malloc(parser, (size))
func YYMALLOC(size) { rb_parser_malloc(parser, size) }
// TODO: follow rb_parser_realloc
#define YYREALLOC(ptr, size)	rb_parser_realloc(parser, (ptr), (size))
func YYREALLOC(ptr, size)	{ rb_parser_realloc(parser, ptr, size) }
// TODO: follow rb_parser_calloc
#define YYCALLOC(nelem, size)	rb_parser_calloc(parser, (nelem), (size))
func YYCALLOC(nelem, size) { rb_parser_calloc(parser, nelem, size) }
// TODO: follow rb_parser_free
#define YYFREE(ptr)		rb_parser_free(parser, (ptr))
func YYFREE(ptr) { rb_parser_free(parser, ptr) }
#define malloc	YYMALLOC
const malloc = YYMALLOC
#define realloc	YYREALLOC
const realloc =	YYREALLOC
#define calloc	YYCALLOC
const calloc = YYCALLOC
#define free	YYFREE
const free = YYFREE
...
// #include "id.c"
// #endif
func REGISTER_SYMID(id int, name string) ID {
	return register_symid(id, name, len(name), enc)
}
...
func is_notop_id(id int) bool { return id > tLAST_OP_ID }
func is_local_id(id int) bool { return is_notop_id(id) && (id & ID_SCOPE_MASK) == ID_LOCAL }
func is_global_id(id int) bool { return is_notop_id(id) && (id & ID_SCOPE_MASK) == ID_GLOBAL }
func is_instance_id(id int) bool { return is_notop_id(id) && (id & ID_SCOPE_MASK) == ID_INSTANCE }
func is_attrset_id(id int) bool { return is_notop_id(id) && ((id)&ID_SCOPE_MASK) == ID_ATTRSET }
func is_const_id(id int) bool { return is_notop_id(id) && ((id) & ID_SCOPE_MASK) == ID_CONST }
func is_class_id(id int) bool { return is_notop_id(id) && ((id) & ID_SCOPE_MASK) == ID_CLASS }
func is_junk_id(id int) bool { return is_notop_id(id) && ((id) & ID_SCOPE_MASK) == ID_JUNK }
func id_type(id int) int { return is_notop_id(id) ? (int)(id & ID_SCOPE_MASK) : -1 }

func is_asgn_or_id(id) bool {
...
// };
const (
    EXPR_BEG_bit = iota		/* ignore newline +/- is a sign. */
    EXPR_END_bit 		/* newline significant +/- is an operator. */
    EXPR_ENDARG_bit		/* ditto and unbound braces. */
    EXPR_ENDFN_bit		/* ditto and unbound braces. */
    EXPR_ARG_bit		/* newline significant +/- is an operator. */
...
// };
type lex_state_e int
const (
    EXPR_BEG lex_state_e = 1 << EXPR_BEG_bit
    EXPR_END lex_state_e = 1 << EXPR_END_bit
    EXPR_ENDARG lex_state_e = 1 << EXPR_ENDARG_bi
    EXPR_ENDFN lex_state_e = 1 << EXPR_ENDFN_bit
    EXPR_ARG lex_state_e = 1 << EXPR_ARG_bit
    EXPR_CMDARG lex_state_e = 1 << EXPR_CMDARG_bit
    EXPR_MID lex_state_e = 1 << EXPR_MID_bit
    EXPR_FNAME lex_state_e = 1 << EXPR_FNAME_bit
    EXPR_DOT lex_state_e = 1 << EXPR_DOT_bit
    EXPR_CLASS lex_state_e = 1 << EXPR_CLASS_bit
    EXPR_VALUE lex_state_e = 1 << EXPR_VALUE_bit
    EXPR_LABELARG lex_state_e = 1 << EXPR_LABELARG_bit
    EXPR_BEG_ANY  lex_state_e =  (EXPR_BEG | EXPR_VALUE | EXPR_MID | EXPR_CLASS | EXPR_LABELARG)
    EXPR_ARG_ANY  lex_state_e =  (EXPR_ARG | EXPR_CMDARG)
    EXPR_END_ANY  lex_state_e =  (EXPR_END | EXPR_ENDARG | EXPR_ENDFN)
)
// #define IS_lex_state_for(x, ls)	((x) & (ls))
// #define IS_lex_state(ls)	IS_lex_state_for(lex_state, (ls))
func IS_lex_state_for(x, ls int) bool	{ return x & ls }
func IS_lex_state(ls int) bool { 	return IS_lex_state_for(lex_state, ls) }
...
type stack_type VALUE

// # define BITSTACK_PUSH(stack, n)	((stack) = ((stack)<<1)|((n)&1))
// # define BITSTACK_POP(stack)	((stack) = (stack) >> 1)
// # define BITSTACK_LEXPOP(stack)	((stack) = ((stack) >> 1) | ((stack) & 1))
// # define BITSTACK_SET_P(stack)	((stack)&1)
func BITSTACK_PUSH(stack *stack_type, n int)	{ stack = (stack<<1)|(n&1) }
func BITSTACK_POP(stack *stack_type)	{ stack = stack >> 1 }
func BITSTACK_LEXPOP(stack *stack_type)	{ stack = (stack >> 1) | (stack & 1) }
func BITSTACK_SET_P(stack *stack_type)	{ stack&1 }

func COND_PUSH(n int)	{ BITSTACK_PUSH(cond_stack, (n)) }
func COND_POP()	{ BITSTACK_POP(cond_stack) }
func COND_LEXPOP()	{ BITSTACK_LEXPOP(cond_stack) }
func COND_P()	{ BITSTACK_SET_P(cond_stack) }

func CMDARG_PUSH(n int)	{ BITSTACK_PUSH(cmdarg_stack, (n)) }
func CMDARG_POP()	{ BITSTACK_POP(cmdarg_stack) }
func CMDARG_LEXPOP()	{ BITSTACK_LEXPOP(cmdarg_stack) }
func CMDARG_P()	{ BITSTACK_SET_P(cmdarg_stack) }
...
var DVARS_INHERIT *vtable = &(vtable{}) // TODO: not sure if this is right
var DVARS_TOPSCOPE *vtable = nil
func DVARS_SPECIAL_P(tbl *vtable) bool { return !POINTER_P(tbl) }
// TODO: I think we don't need this in Go
func POINTER_P(val *vtable) bool { return VALUE(val) & ~VALUE(3) }
...
//     return tbl;
// }
func newVtable(prev *vtable) *vtable {
  tbl := vtable{
    pos: 0,
    capa: 8,
    prev: prev,
  ...
  // TODO: follow ALLOC_N
  // tbl.tbl = ALLOC_N(ID, tbl.capa);
  if VTBL_DEBUG { fmt.Printf("vtable_alloc: %p\n", &tbl) }
  return &tbl
}
...
//     }
// }
func (tbl *vtable) free() {
  if VTBL_DEBUG { fmt.Printf("vtable_free: %p\n", &tbl) }
  if tbl != nil {
    if tbl.tbl != nil {
      tbl.tbl = nil
    }
...
//     tbl->tbl[tbl->pos++] = id;
// }
func (tbl *vtable) add(id ID) {
	if tbl == nil {
	   rb_bug("vtable_add: vtable is not allocated (%p)", tbl);
	}
	if (VTBL_DEBUG) { fmt.Printf("vtable_add: %p, %s\n", tbl, rb_id2name(id)) }
	if tbl.pos == tbl.capa {
		tbl.capa = tbl.capa * 2
		// TODO: follow REALLOC_N
		// REALLOC_N(tbl->tbl, ID, tbl->capa);
	}
	tbl.tbl[tbl.pos++] = id;
}
...
//     return 0;
// }
func (tbl *vtable) included(id ID) {
	if (tbl != nil) {
		for _, tblId := range tbl {
			if tblId == id {
				return true
			}
		}
...
//     parser_lex_pbeg string
//     parser_lex_p string
//     parser_lex_pend string
//     int parser_heredoc_end;
//     int parser_command_start;
//     parser_deferred_nodes *NODE
...
// #endif
// };
type parser_params struct {
    is_ripper int
    heap *NODE

    parser_yylval *YYSTYPE
    eofp VALUE

    parser_lex_strterm *NODE
    parser_lex_state lex_state_e
    parser_cond_stack stack_type
    parser_cmdarg_stack stack_type
    parser_class_nest int
    parser_paren_nest int
    parser_lpar_beg int
    parser_in_single int
    parser_in_def int
    parser_brace_nest int
    parser_compile_for_eval int
    parser_cur_mid VALUE
    parser_in_kwarg int
    parser_in_defined int
    parser_tokenbuf string
    parser_tokidx int
    parser_toksiz int
    parser_tokline int
    parser_lex_input VALUE
    parser_lex_lastline VALUE
    parser_lex_nextline VALUE
    parser_lex_pbeg string
    parser_lex_p string
    parser_lex_pend string
    parser_heredoc_end int
    parser_command_start int
    parser_deferred_nodes *NODE
    parser_lex_gets_ptr long
    parser_lex_gets func(*parser_params, VALUE) VALUE
    parser_lvtbl *local_vars
    parser_ruby__end__seen int
    line_count int
    has_shebang int
    parser_ruby_sourcefile *string /* current source file */
    parser_ruby_sourceline int /* current line no. */
    parser_ruby_sourcefile_string VALUE
    enc *rb_encoding
...
// #ifndef RIPPER
    /* Ruby core only */
    parser_eval_tree_begin *NODE
    parser_eval_tree *NODE
    debug_lines VALUE
    coverage VALUE
    nerr int

    parser_token_info_enabled int
    parser_token_info *token_info
// #else
    /* Ripper only */
    tokp string
    delayed VALUE
...
// TODO: follow rb_enc_str_new
func STR_NEW(p,n) { rb_enc_str_new((p), (n), current_enc) }
#define STR_NEW0() rb_enc_str_new(0,0,current_enc)
#define STR_NEW2(p) rb_enc_str_new((p),strlen(p),current_enc)
#define STR_NEW3(p,n,e,func) parser_str_new((p),(n),(e),(func),current_enc)
#define ENC_SINGLE(cr) ((cr)==ENC_CODERANGE_7BIT)
...
{
    int cnt_args = lvtbl.args.size();
    int cnt_vars = lvtbl.vars.size();
...
local_id_gen(struct parser_params *parser, ID id)
{
    ...
    else if (args.included(id)) {
	return 1;
    }
    else {
	int i = vars.included(id);
...
{
    return (lvtbl.args.included(id) ||
	    lvtbl.vars.included(id));
}
...
{
    ID id = ID(lvtbl.args.size()) + ID(lvtbl.vars.size());
